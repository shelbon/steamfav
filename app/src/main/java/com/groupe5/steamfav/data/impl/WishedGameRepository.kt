...
import com.groupe5.steamfav.data.abstraction.WishlistGameRepository
...
class WishedGameRepository(private val firebaseStore: FirebaseFirestore) : WishlistGameRepository {
    private var collectionName: String = CollectionNames.WISHLIST.trueName

    override fun addGameToWishList(likedGame: LikedGame) = flow {
    ...
    override fun addUserToWishlist(likedGameId: Long, gameName: String, userId: String) = flow {
        ...
        emit(
            NetworkResult.Error(
                it.localizedMessage ?: "some error happened",
                false
            )
        )
    }

    override fun removeGameToWishlist(likedGameId: Long, userId: String): Flow<NetworkResult<Int>> =
        flow {
            val snapshot =
                firebaseStore.collection(collectionName)
                    .whereEqualTo("appid", likedGameId)
            ...
            documents.documents.let { documentsSnapshot ->
                if (documentsSnapshot.isNotEmpty()) {
                    ...
                    emit(
                        NetworkResult.Error(
                            Resources.getSystem().getString(R.string.remove_liked_failed_not_found),
                            0
                        )
                    )
                }
